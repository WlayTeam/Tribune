<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%><!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd"><html><body><div class="entry">
 <div class="copyright-area">
  原文出处：
  <a href="http://mccxj.github.io/blog/20130615_java-string-constant-pool.html" ref="nofollow" target="_blank">
   小毛的胡思乱想
  </a>
 </div>
 <div>
  ...
  <pre class="brush: java; gutter: true">String m = "hello,world";
String n = "hello,world";
String u = new String(m);
String v = new String("hello,world");</pre>
  <p>
   这些语句会发生什么事情? 大概是这样的:
  </p>
  <ol>
  ...
  <p>
   <img alt="对象在内存中的布局" src="http://mccxj.github.io/assets/images/2013/string/string1.png"/>
  </p>
  <p>
...
public void test1() throws Exception {
  ...
  <p>
   另外，字符串常量通常是在编译的时候就确定好的，定义在类的方法区里边，也就是说，不同的类，即使用了同样的字符串， 还是属于不同的对象。所以才需要
   <strong>
    通过引用字符串常量来减少相同的字符串的数量
   </strong>
   。可以通过下面的代码来测试一下：
  </p>
  <pre class="brush: java; gutter: true">class A {
    public void print() {
...
String v = u.substring(4,7);</pre>
  <p>
   按照上面的说法，m,n的数据结构就如下图所示:
  </p>
  <p>
   <img alt="substring在内存中的布局" src="http://mccxj.github.io/assets/images/2013/string/string2.png"/>
  </p>
  <p>
  ...
  <pre class="brush: java; gutter: true">String m = "hello,";
String u = m.concat("world");
String v = new String(m.substring(0,2));</pre>
  <p>
   得到的结构图如下:
  </p>
  <p>
   <img alt="新字符数组在内存中的布局" src="http://mccxj.github.io/assets/images/2013/string/string3.png"/>
  </p>
  <p>
  ...
  <pre class="brush: java; gutter: true"> javap -v StringTest
...
+ const #47 = Asciz       world;
 ...</pre>
  ...
  <pre class="brush: java; gutter: true">String m = "hello,world";
String u = m + ".";
String v = "hello,world.";</pre>
  <p>
   u和v虽然是一样内容的字符串，但内部的字符数组不是同一个。画成图的话就是这样的:
  </p>
  <p>
   <img alt="不同字符数组在内存中的布局" src="http://mccxj.github.io/assets/images/2013/string/string4.png"/>
  </p>
  <p>
   另外有一点，如果让m声明为final，你就会发现u和v变成是同一个对象。画成图的话就是这样的:
  </p>
  <p>
   <img alt="u和v在内存中的布局" src="http://mccxj.github.io/assets/images/2013/string/string5.png"/>
  </p>
  <p>
   这应该怎么解释的？这其实都是编译器搞的鬼，因为m是final的， u直接被编译成”hello,world.”了，如果使用javap查看的话，会发现下面一段逻辑:
  </p>
  <pre class="brush: java; gutter: true">const #2 = String       #25;    //  hello,world
...
n = null;</pre>
  ...
  <p>
   <img alt="内存释放后的布局" src="http://mccxj.github.io/assets/images/2013/string/string7.png"/>
  </p>
  <p>
  ...
  <ul>
   <li>
    <a class="external" href="http://blog.xebia.com/2007/10/04/leaking-memory-in-java/" rel="nofollow" target="_blank">
     Leaking Memory in Java
    </a>
   </li>
   <li>
    <a class="external" href="http://www.iteye.com/topic/626801" rel="nofollow" target="_blank">
